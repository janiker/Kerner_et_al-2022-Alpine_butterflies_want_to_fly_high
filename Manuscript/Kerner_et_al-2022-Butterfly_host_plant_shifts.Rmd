...
Open Research: Data and code are available on GitHub under: [https://github.com/janiker/Kerner_et_al-2022-Butterfly_host_plant_shifts] and will be stored permanently via Dryad after manuscript acceptance.

**keywords:** Alps; altitudinal gradient; global warming; grazing; Lepidoptera; mountain biodiversity; plant-herbivore interactions; species range shifts; upslope shift
...
### mean shift model of all species (>= 10 individuals per species in each year)
...
### linear model, with individuals as data points to check significance for single species

B<-read.csv(here::here("data", "B.csv"), row.names=1) # load individual elevations all species
...
### check for signifikant difference in shifts between alpine and non-alpine species
...
### mean shift model of all species (>= 10 occupied subplots per species in each year)
...
### linear model, with individuals as data points to check significance for single species
st<-read.csv(here::here("data", "st.csv"), row.names=1) # load individual elevations all species
...
allbutt.shifts<-read.csv(here::here("data","allbutt.shifts.csv"),row.names = 1)
...
# mean elevation per species with Wilcoxon test results
sub.shifts<-read.csv(here::here("data","sub.shifts.csv"),row.names = 1)
...
# plotting of temperature graph (Figure 1b)
...
qgraph(bd,shape="square",layout=l1,labels=F, vsize=18,labels=F, edge.color="black", edge.label.cex=1.8,asize=6, images=c(here::here("figures","Logo_Gen_ver.png"),here::here("figures","Logo_hostplant.png"),here::here("figures","Logo_Ere_eur.png"),here::here("figures","Logo_temp.png")), borders=F);text(-1.1,1.1,"c)",cex=1.3); text(
...
z<-read.csv(here::here("data","z.csv"), sep=";") # read species shifts with significance symbols
...
s<-read.csv(here::here("data","s.csv"), sep=";") # read species shifts with significance symbols
colnames(s)[1]<-"abbr"
...
```{r butthostplantlist, tab.cap="List of included butterfly species and associated larval host plants according to Paolucci (2013) and/or Reinhardt et al. (2020). The degree of specialization (deg. spec.) states if the larvae of the species is monophagous (M), oligophagous (O) or polyphagous (P) according to Paolucci (2013). Inconsistencies between the given degree of specialization and the named associated larval host plants derive from the use of Reinhardt et al. (2020) as supplementing literature for the associated plant species.", echo=FALSE, warning=FALSE}
...
include_graphics(here::here("figures","climate_stations_temperature.jpeg"))
```
