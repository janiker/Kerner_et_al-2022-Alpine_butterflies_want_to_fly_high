...
shift<-read.csv(here::here("Data", "shift.csv"), row.names=1)
...
# comparison single species - elevation subplot based ------------------------------------

plants09<-read.table("J:/Daten_Annette&Bernhard/Plants_2009_Bernhard/Alpine_plants_2009_species.records-1st-oT3.txt", skipNul = T, header = T, dec=",")
colnames(plants09)[1]<-"plot"
plants09$spec<-as.factor(plants09$spec);plants09$plot<-as.factor(plants09$plot);plants09$abbr<-as.factor(plants09$abbr)
plants09<-subset(plants09,!is.na(cov)) # kick NAs

plants19<-read.table("J:/Auswertung/Textfiles/plantspecies19-1.txt",header=T)
plants19<-subset(plants19,plants19$plot!="T2")
plants19$abbr<-as.factor(plants19$abbr);plants19$plot<-as.factor(plants19$plot)
plants19<-subset(plants19,!is.na(cov)) # kick NAs

# add elevation
plotdata3<-read.table("J:/Auswertung/Textfiles/plot_attributes.txt",header=T)
plotdata3<-subset(plotdata3,plot!="T3")

plants09<-left_join(plants09,plotdata3[,1:2],by="plot")
plants09$year<-as.factor(2009)
plants19<-left_join(plants19,plotdata3[,1:2],by="plot")
plants19$year<-as.factor(2019)

# summary per subplot
t09<-as.data.frame(table(plants09$abbr,plants09$subplot))
t09$Freq[t09$Freq>1]<-1 # kick double assessed in Subplot
t09<-separate(t09,Var2,into=c("plot","subplot"))
t09<-as.data.frame.matrix(table(t09$Var1[t09$Freq>0],t09$plot[t09$Freq>0]))
t09<-t(t09)

t19<-as.data.frame(table(plants19$abbr,plants19$subplot))
t19$Freq[t19$Freq>1]<-1 # kick double assessed in Subplot
t19<-separate(t19,Var2,into=c("plot","subplot"))
t19<-as.data.frame.matrix(table(t19$Var1[t19$Freq>0],t19$plot[t19$Freq>0]))
t19<-t(t19)

# Arten mittlere Höhe & Gesamtdeckung/Anzahl Subplots, um >= x Subplots auswählen zu können
wsb09<-colSums(t09*plotdata3$elev)/colSums(t09)
wsb09<-as.data.frame(wsb09);wsb09<-rownames_to_column(wsb09)
colnames(wsb09)<-c("abbr","melev")
wsb09$no.sb<-colSums(t09)
wsb09$year<-2009

#um Vorzeichen von signifikanten Unterschieden zu bestimmen auch 2019er Werte rechnen
wsb19<-colSums(t19*plotdata3$elev)/colSums(t19)
wsb19<-as.data.frame(wsb19);wsb19<-rownames_to_column(wsb19)
colnames(wsb19)<-c("abbr","melev")
wsb19$no.sb<-colSums(t19)
wsb19$year<-2019

wsb<-merge(wsb09[1:3],wsb19[1:3],by="abbr");colnames(wsb)<-c("abbr","2009","no.sb09","2019","no.sb19") #werden nur Arten behalten, die in beiden Jahren vorkamen
wsb$diff<-wsb$`2019`-wsb$`2009`
wsb$vorzeichen[wsb$diff<0]<-"-"
wsb$vorzeichen[wsb$diff>0]<-"+"
wsb$vorzeichen[wsb$diff==0]<-"0"

#Subset auf Hostplants
hp<-read.table("J:/Auswertung/Textfiles/But_hostplants_species.txt",header=T)
hp<-hp[-c(1:3),];hp<-hp$spec
subraus<-subset(wsb[wsb$abbr %in% hp,],select = abbr) 
subraus<-as.character(subraus$abbr)
wsb<-wsb[wsb$abbr %in% hp,]

#mittlere Höhe 09~19 pro Art testen
st09<-aggregate(elev~abbr+subplot,data=plants09,FUN=mean);st09$year<-2009 # damit für jede Art jeder Subplot nur einmal vorkommt
st19<-aggregate(elev~abbr+subplot,data=plants19,FUN=mean);st19$year<-2019

sub.wilcox<-as.data.frame(subraus);sub.wilcox$p.value<-0;sub.wilcox$no<-1:nrow(sub.wilcox);colnames(sub.wilcox)[1]<-"abbr"

for (i in subraus){
  cat(paste(subset(sub.wilcox,abbr==i)[,3],"von",length(subraus),"\n"))
  Ber09<-subset(st09,st09$abbr==i)
  Luk19<-subset(st19,st19$abbr==i)
  specci<-bind_rows(Ber09[,3:4],Luk19[,3:4])
  sub.wilcox$p.value[subset(sub.wilcox,abbr==i)[,3]]<-wilcox.test(specci$elev~specci$year)$p.value
}

sub.wilcox<-left_join(sub.wilcox,wsb,by="abbr")


#subset auf Hostplants mit mind. 10 Subplots
subraus<-subset(wsb[wsb$abbr %in% hp & wsb$no.sb09>=10 &wsb$no.sb19>=10,],select = abbr)
subraus<-as.character(subraus$abbr)

# mean shift model of all/significant species (>= 10 individuals)
sshift<-bind_rows(wsb09,wsb19); sshift$year<-as.factor(sshift$year)
sshift<-sshift[sshift$abbr %in% subraus,] # subset auf Arten >= 10 Subplots & sshift$abbr %in% sub.wilcox$abbr[sub.wilcox$p.value<=0.05] für subset auf signifikante Arten

library(lme4)
xx<-summary(lmer(melev ~ year+(1|abbr), data=sshift)) # Werte gleich wie pss & pss.sd
...
sd<-st19[,c(1,3)];sd<-subset(sd,sd$abbr %in% subraus) # Arten von einem Jahr, weil die die fehlen eh egal sind

for (i in subraus) { # alle 2019er Werte minus den 2009er Wert, um Shift für jeden Einzelwert aus 2019 zu bekommen
  sd$dif09[sd$abbr==i]<-sd$elev[sd$abbr==i]-wsb09$melev[wsb09$abbr==i]
}

s <- sd %>% 
  group_by(abbr) %>% 
  summarise_each(funs(mean, sd, n()), dif09) %>% 
  mutate(se = sd/sqrt(n),
         left95 = mean - se,  # standard error: möglicher Raum, in dem Mittelwert liegen kann, deviation = Varianz der Daten
         right95 = mean + se)


s$signcol<-ifelse(s$abbr %in% sub.wilcox$abbr[sub.wilcox$p.value<=0.05], "signif", "ns")
s$col<-s$signcol;s$col[s$col=="signif"]<-"black";s$col[s$col=="ns"]<-"grey80"

s$signlvl<-NA
s$signlvl[s$abbr %in% sub.wilcox$abbr[sub.wilcox$p.value<=0.05]]<-"*"
s$signlvl[s$abbr %in% sub.wilcox$abbr[sub.wilcox$p.value<=0.01]]<-"**"
s$signlvl[s$abbr %in% sub.wilcox$abbr[sub.wilcox$p.value<=0.001]]<-"***"

plantnames<-read.table("J:/Auswertung/Textfiles/plantnames-1.csv",header = T,sep=";")
s$taxon<-NA
s$abbr<-as.character(s$abbr);s$abbr<-as.factor(s$abbr)

for (i in levels(s$abbr)){
  s$taxon[s$abbr==i]<-plantnames$taxon[plantnames$abbr==i]
}
```

```{r shiftselect.butt.calc, include=FALSE}

# Uebersicht mean elevation shift nach min. Anzahl Individuen und enthaltener Artenzahl

a<-as.data.frame(5:30);colnames(a)<-"no.ind"
a$mean<-0; a$no.species<-0
a$n.s.<-0;a$cs<-0;a$s<-0;a$ss<-0;a$sss<-0

# butt.wilcox neu auch mit Arten unter 10 Ind.
allbutt.wilcox<-as.data.frame(spwm$spec); colnames(allbutt.wilcox)<-"spec"
allbutt.wilcox$p.value<-0;allbutt.wilcox$no<-1:nrow(allbutt.wilcox)

for (i in levels(as.factor(allbutt.wilcox$spec))){
  Ann09<-subset(B09,B09$genus_species==i)
  Jan19<-subset(B19,B19$genus_species==i)
  specci<-bind_rows(Ann09[,11:12],Jan19[,12:13])
  allbutt.wilcox$p.value[subset(allbutt.wilcox,spec==i)[,3]]<-wilcox.test(specci$elev~specci$year)$p.value
}

allbutt.wilcox<-left_join(allbutt.wilcox,spwm,by="spec")
...
library(MuMIn) #AICc berechnen
library(nlme) # lme
library(piecewiseSEM) # SEMs

# Daten einlesen --------------------------------------------------------------

s66<-read.table("J:/Auswertung/Textfiles/SEM-66.csv",header=T,sep=";")
s66$plot<-as.factor(s66$plot)
s66<-s66[s66$plot!="T3",] #T3 raus nehmen
s66$flow_cov<-log(s66$flow_cov) #als log für Normalverteilung

# SEM 66er summer temp ---------------------------------------------------------
...
ss1<-summary(sem1) # marginal effects: only fixed, conditional: recognizing the random factors

# Punkt als marginal signifikantes Symbol einfügen
for (i in 1:8) {
...
sdel<-read.table("J:/Auswertung/Textfiles/SEM-delta.csv",header=T,sep=";")
sdel$plot<-as.factor(sdel$plot)
sdel<-sdel[sdel$plot!="T3",] #T3 raus nehmen
sdel$flow_cov<-sqrt(sdel$flow_cov+3) # sqrt für annähernd Normalverteilung, +3 damit alle Werte positiv sind und transformiert werden können

# 3 min. Modelle, alles fällt raus, außer Direkteinflüsse auf delta
# in allen drei Modellen nur temps/tempw signifikant
...
# Temperaturunterschied pro 100 m auf Basis der gemessenen Klimastationsdaten

LT<-read.csv("J:/Auswertung/Textfiles/Klimastationen_TempDaymeans.csv", sep=",")
LT<-LT[LT$year.x>=2009 & LT$year.x>=2009,]
library(nlme)
tc100<-summary(lme(daymeanT~Elev_50m, random=~1|Stationsname, data=LT))
tccoef<-tc100[["coefficients"]][["fixed"]][["Elev_50m"]]*-1

# Unterschied Jahresmittel 2009 - 2019

yearmean<-aggregate(daymeanT~year, data=climate[climate$year>="2009" & climate$year <="2019",], FUN=mean)

tmean<-summary(lm(daymeanT~year,yearmean))


# Unterschied Jahresmittel Sommer 2009 - 2019
summean<-aggregate(daymeanT~year, data=climate[climate$daymonth >="2022-05-01" & climate$daymonth <="2022-09-30" & climate$year>="2009" & climate$year <="2019",],FUN=mean)

stmean<-summary(lm(daymeanT~year,data=summean))


# Unterschied 2009 / 2019
...
# plot specific temp calculations

plotmeany<-aggregate(daymeanT~year, data=climate[climate$year>="2009" & climate$year<="2019" & climate$plot=="H1",],FUN=mean)

plotmeanw<-plotmeany; plotmeans<-plotmeany
...
for (i in sdel$plot){
  ...
  # Jahresmittel Jan - Dez
   plotmeany[,n]<-aggregate(daymeanT~year, data=climate[climate$year>="2009" & climate$year<="2019" & climate$plot==i,],FUN=mean)[,2]
  colnames(plotmeany)[n]<-i
  sdel$ptempy[sdel$plot==i]<-lm(plotmeany[,i]~plotmeany$year)$coefficients[2]*10
  
  for (k in 2009:2019){
    plotmeanw[plotmeanw$year==k,n]<-mean(climate$daymeanT[((climate$daymonth>="2022-10-01"&climate$year==(k-1))|(climate$daymonth<="2022-04-30"&climate$year==k)) & climate$plot==i])
    }
  colnames(plotmeanw)[n]<-i
  sdel$ptempw[sdel$plot==i]<-lm(plotmeanw[,i]~plotmeanw$year)$coefficients[2]*10
 
  plotmeans[,n]<-aggregate(daymeanT~year, data=climate[climate$year>="2009" & climate$year<="2019" & climate$plot==i & climate$daymonth >="2022-05-01" & climate$daymonth <="2022-09-30",],FUN=mean)[,2]
  colnames(plotmeans)[n]<-i
  sdel$ptemps[sdel$plot==i]<-lm(plotmeans[,i]~plotmeans$year)$coefficients[2]*10
  
}

# GAM Jahres- & Sommermittel 2009 / 2019

gst<-summary(gam(temps~s(elev, k=4), data=sdel, method="REML"))
gyt<-summary(gam(tempy_old~s(elev, k=4), data=sdel, method="REML"))
...
psign<-function(p){
if(p<=0.001) {
  p <- "***" #Signifikanzwert Jahreseffekt
  } else if (p<=0.01) {
    p <- "**"
    } else if (p<=0.05) {
      p <- "*"
...
options(scipen = 999) # Zahlen dezimal und nicht expotential (e+06) darstellen

GAMT<-data.frame(#logos = c("J:/Layout/Logo_temp.png",
                           #"J:/Layout/Logo_temp.png", 
                           #"J:/Layout/Logo_temp.png",
                           #"J:/Layout/Logo_temp.png",
                           #"J:/Layout/Logo_Ere_eur.png", NA, NA, 
                           #"J:/Layout/Logo_Ere_eur.png", NA, NA, 
                           #"J:/Layout/Logo_hostplant.png",NA, NA,
                           #"J:/Layout/Logo_Ere_eur.png", NA, NA),
                 resp.var = c("MAT 2009 / 2019",
                              "MST 2009 / 2019",
                              "MAT 2009 - 2019",
                              "MST 2009 - 2019",
...
# welche Spalten in Tabelle gezeigt werden sollen  
...
# Spalten Logos & Rownames in eine zusammenfügen und in einer Spalte plotten
   #compose( 
   #j = "resp.var", i = c(1:5,8,11),
    #value = as_paragraph(
      #as_image(src = logos, width = .5, height = .5), " ",
      #as_b(x = resp.var)
      #)) %>%
 # width(j = "resp.var", width = 4) # Spaltenbreite verändern

# Logos direkt plotten, funktioniert leider nicht mit Word
  #colformat_image(j = "logos", i = c(1:5, 8, 11), width = .5, height = .5) # Bilder als Grafik in Tabelle drucken

# direkt Summary-Tabelle aus GAM-Objekt
#as_flextable(gam(spec.no~s(elev)+s(elev,by=year)+s(plot, bs="re"), data=pdl,family=gaussian,method="REML"))
```
...
library(svglite) # um svg exportieren zu können

ggsave("J:/Auswertung/Abbildungen/Paper_I/tempchange.svg", plot =(

  sdel %>% # mit Jahresmittel Okt - Sept
  pivot_longer(cols=c(temps,ptemps,tempy_old,ptempy),names_to = c("time"),values_to="temp") %>% 
ggplot(aes(x=elev, y=temp, col=time)) +
  geom_hline(yintercept = 0,col="ivory3")+ # Nulllinie
  geom_point(size=3,alpha=.4,shape=16)+
  geom_smooth(method = gam, formula = y ~ s(x, k=4), size = 2, se = F, alpha=.2) +
  theme_classic()+ # weißer Hintergrund
  theme(# panel.background = element_rect(colour = "black", size=0.5), # schwarzer Rahmen
    plot.margin = margin(t = 10, r = 10, b = 10, l = 10),  # Abstand zum Bildrand
    legend.position = c(0.75,0.2), # Position Legende, geht außerhalb vom Plot auch mit "bottom" etc.
    legend.title=element_text(size=14,margin = margin(l = -6)), # Legendenbeschriftung, margin mit Minus, um ihn nach links zu verschieben
    legend.text = element_text(size=14), # Schriftgröße Legendentext
    legend.key.height = unit(0.2, "cm"), # Zeilenabstand zwischen Legendenitems
    legend.background = element_blank(),
    axis.text = element_text(size=20), # Schriftgröße Achsenbeschriftung
    axis.title.x = element_text(size=20,margin = margin(t = 10, r = 0, b = 0, l = 0)), # Schriftgröße & Abstand Achsenlabels,
    axis.title.y = element_text(size=20,margin = margin(t = 0, r = 10, b = 0, l = 0)))+
  labs(x="elevation [m.a.s.l.]",y=expression(paste(Delta, " temperature [°C]")))+
  scale_x_continuous(breaks=c(750,1000,1250,1500,1750,2000))+ # für unbeschriftete Markierungen: labels = c("",1000,"",1500,"",2000)
  scale_y_continuous(n.breaks=4)+
  scale_color_manual(name="MST  MAT", values = c("temps"="violetred4", "ptemps"="#CC8FAB", "tempy_old"="navyblue", "ptempy"="#7D7DB3"),labels=c(" "," ","year difference\n(2009/2019)","long term trend\n(2009-2019)"),
  guide=guide_legend(ncol=2))+
  annotation_custom(
    grid::rasterGrob(
      image = readPNG("J:/Layout/Logo_temp.png"),
      x = .05, y =.9,
      width =.15))
...
library(knitr)
include_graphics("J:/Auswertung/Abbildungen/Paper_I/Verortung_Plots2019_ohneDE_adj.png")
```
...
gg<-bind_rows("spec.no"=moddivy,"div.hp"=moddipy, .id = "group") %>% # Tabellen zusammenfügen, damit spec.no & div.hp untereinander sind
ggplot(aes(x=elev, y=spec.no, colour=as.factor(year), fill=as.factor(year))) +
  facet_wrap(vars(fct_inorder(group)),ncol = 1, scales = "free") + # scales = free, um unterschiedliche Skalen pro Plot zuzulassen bzw. bei x um Markierungsstriche zu erhalten
  geom_ribbon(aes(ymin = CI_lower, ymax = CI_upper), alpha = 0.2, colour = NA) + # colour=NA, um Außenkonturen zu entfernen
  geom_line(size=2)+
  geom_point(data=s66 %>%
  ...
  theme(# panel.background = element_rect(colour = "black", size=0.5), # schwarzer Rahmen
    ...
    legend.position = c(0.7,0.97), # Position Legende, geht außerhalb vom Plot auch mit "bottom" etc.
    legend.title=element_blank(), # Legendenbeschriftung raus
    legend.text = element_text(size=14), # Schriftgröße Legendentext
    axis.text = element_text(size=14), # Schriftgröße Achsenbeschriftung
    axis.title.x = element_text(size=14,margin = margin(t = 10, r = 0, b = 0, l = 0)), # Schriftgröße & Abstand Achsenlabels,
    axis.title.y = element_text(size=14,margin = margin(t = 0, r = 10, b = 0, l = 0)),
    strip.text = element_blank())+ # Labelbalken über Plots durch Facetwrap entfernen
  ...
  geom_text(inherit.aes = FALSE, data=tags, aes(x= -Inf, y=Inf, label=lab),hjust=-1,vjust=1, size=5) # Facet labels einfügen
...
logos <- list(rasterGrob(readPNG("J:/Layout/Logo_Ere_eur.png"),
                             width=.15, x = .9, y = .92),
                  rasterGrob(readPNG("J:/Layout/Logo_hostplant.png"),
                             width=.15, x = .9, y = .93))
...
library(qgraph) # SEMs schön plotten
library(png)
library(grid) # arrange png-files
library(gridExtra)

# manuelle matrix mit Estimates aus psem für Darstellung
...
#Diese Matrix listet wo in einer 7x7 Matrix die column names aus der Tabelle t platziert werden sollen
...
signlabels<-c( # Werte direkt aus summary nehmen, fehlt allerdings "." & n.s., wird "" angezeigt; Reihenfolge nach Nr. von Response
...
dsignlabels<-c( # Werte direkt aus summary nehmen, fehlt allerdings "." & n.s., wird "" angezeigt; Reihenfolge nach Nr. von Response
...
# Basismodell s66
png("J:/Auswertung/Abbildungen/Paper_I/sem1.png", res=600, height=8, width=8, units = "cm")

qgraph(b,shape="square",layout=l1,labels = F, vsize=18,labels=F, edge.color="black", edge.label.cex=1.8, asize=6, images=c("J:/Layout/Logo_Gen_ver.png","J:/Layout/Logo_hostplant.png","J:/Layout/Logo_Ere_eur.png","J:/Layout/Logo_temp.png","J:/Layout/Logo_man.png"), borders=F) ;text(-1.1,1.1,"a)",cex=1.3);abline(-1.3,0, lwd=2, col="slategray4") # asize = Dicke Pfeilspitzen, label.norm, damit alle Beschriftungen gleich groß sind, label.prop = Verhältnis Quadrate zu Text; abline für Zwischenlinie zu unteren Plots (c, d)

dev.off()

# Pfadmodell s66
png("J:/Auswertung/Abbildungen/Paper_I/sem2.png", res=600, height=8, width=8, units = "cm")

qgraph(t,shape="square", layout=l1, labels=F, vsize=18, edge.color=c("slategray2","black","slategray2","black", "slategray2","black","black","slategray2"), colFactor=0, asize=6, edge.labels=signlabels, edge.label.bg=T, edge.label.cex=1.8, edge.label.margin=0.025, lty=c(1,1,2,1,1,1,1,1), images=c("J:/Layout/Logo_Gen_ver.png","J:/Layout/Logo_hostplant.png","J:/Layout/Logo_Ere_eur.png","J:/Layout/Logo_temp.png","J:/Layout/Logo_man.png"), borders = F # borders = Kästchenrand
...
# Basismodell delta
png("J:/Auswertung/Abbildungen/Paper_I/sem3.png", res=600, height=8, width=8, units = "cm")

qgraph(bd,shape="square",layout=l1,labels=F, vsize=18,labels=F, edge.color="black", edge.label.cex=1.8,asize=6, images=c("J:/Layout/Logo_Gen_ver.png","J:/Layout/Logo_hostplant.png","J:/Layout/Logo_Ere_eur.png","J:/Layout/Logo_temp.png"), borders=F);text(-1.1,1.1,"c)",cex=1.3) # asize = Dicke Pfeilspitzen, label.norm, damit alle Beschriftungen gleich groß sind

dev.off()

#Pfadmodell delta
png("J:/Auswertung/Abbildungen/Paper_I/sem4.png", res=600, height=8, width=8, units = "cm")

qgraph(d,shape="square",layout=l1,vsize=18, labels=F, edge.color=c("slategray3","black"),colFactor=0,asize=6,edge.labels=dsignlabels,edge.label.bg=T, edge.label.cex=1.8, edge.label.margin=0.025, images=c("J:/Layout/Logo_Gen_ver.png","J:/Layout/Logo_hostplant.png","J:/Layout/Logo_Ere_eur.png","J:/Layout/Logo_temp.png"), borders=F
...
path1<-readPNG("J:/Auswertung/Abbildungen/Paper_I/sem1.png")
path2<-readPNG("J:/Auswertung/Abbildungen/Paper_I/sem2.png")
path3<-readPNG("J:/Auswertung/Abbildungen/Paper_I/sem3.png")
path4<-readPNG("J:/Auswertung/Abbildungen/Paper_I/sem4.png")
```
...
ggplot(modC,aes(x=elev, y=comsc, colour=year, fill=year)) +
  ...
  theme(# panel.background = element_rect(colour = "black", size=0.5), # schwarzer Rahmen
    plot.margin = margin(t = 10, r = 10, b = 10, l = 10), # Abstand zum Bildrand
    legend.position = c(0.1,0.8), # Position Legende, geht außerhalb vom Plot auch mit "bottom" etc.
    legend.title=element_blank(), # Legendenbeschriftung raus
    legend.text = element_text(size=14), # Schriftgröße Legendentext
    legend.background = element_blank(),
    axis.text = element_text(size=14), # Schriftgröße Achsenbeschriftung
  ...
  scale_x_continuous(breaks=c(800,1200,1600,2000))+ # für unbeschriftete Markierungen: labels = c("",1000,"",1500,"",2000)
  scale_y_continuous(n.breaks=6)+
  scale_fill_manual(name="year", values = c("2009"="dodgerblue", "2019"="orange"))+
  scale_color_manual(name="year", values = c("2009"="dodgerblue", "2019"="orange"))+
annotation_custom(
    grid::rasterGrob(
      image = readPNG("J:/Layout/Logo_Ere_eur.png"),
      x = .1, y =.93,
      width =.15))
```
...
z$signpos<--350 # Position Sternchen

z$genus_species<-revalue(z$genus_species, c("Leptidea sinapis"="Leptidea sinapis agg.", "Pieris bryoniae"="Pieris bryoniae agg."))

txtcol<-revalue(z$signcol[order(z$mean)], c(ns="grey80",signif="black", alpns="paleturquoise3", alpsignif="turquoise4")) # Farbe y-Achsenlabels, nach mean sortiert

buttshifts<-ggplot(z, aes(mean,reorder(genus_species,mean),col=signcol)) + #mit reorder Reihenfolge der Darstellung ändern
  #z[z$genus_species %in% butt.wilcox$spec[butt.wilcox$p.value<=0.05],], aes(mean,reorder(genus_species,mean),col=signcol))+
  geom_vline(xintercept = 0,linetype="dashed")+
  geom_pointrange(aes(xmin=left95,xmax=right95),shape="|",size=.7,stroke=1)+
  theme(
    panel.background = element_rect(fill = NA), # Grafikhintergrund weiß
    legend.position = "top", # Position Legende, geht außerhalb vom Plot auch mit "bottom" etc.
    legend.title=element_text(size=9), # Legendenbeschriftung
    legend.text = element_text(size=9), # Schriftgröße Legendentext
    legend.key.height = unit(0.5, "cm"), # Zeilenabstand zwischen Legendenitems
    legend.background = element_blank(),
    axis.line = element_line(size = 0.5), # Achsenlinien zeichnen
    axis.ticks.y = element_blank(), # Achsenmarkierungen entfernen
    axis.text.y = element_text(face = "italic", colour=txtcol),
    axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0)), # Schriftgröße & Abstand Achsenlabels,
    axis.title.y = element_blank() # Schriftgröße & Abstand Achsenlabels,
  )+
  ...
  geom_text(aes(x=signpos,label = signlvl), vjust=0.8, hjust=0, show.legend = F)+ #vjust auf selbe Höhe wie Linie, hjust = linksbündig, 
...
s$signpos<--267 # Position für Sternchen in Grafik

plantshifts<-ggplot(s, aes(mean,reorder(abbr,mean),col=signcol)) + # alle Arten, mit reorder Reihenfolge der Darstellung ändern
  #s[s$abbr %in% sub.wilcox$abbr[sub.wilcox$p.value<=0.05],], aes(mean,reorder(abbr,mean),col=signcol))+ #nur signifikante Arten
  geom_vline(xintercept = 0,linetype="dashed")+ # Nulllinie
  geom_pointrange(aes(xmin=left95,xmax=right95),shape="|",size=0.5,stroke=1)+ # Ranges einzeichnen
  theme(
    panel.background = element_rect(fill = NA), # Grafikhintergrund weiß
    axis.line = element_line(size = 0.5), # Achsenlinien zeichnen
    axis.ticks.y = element_blank(), # Achsenmarkierungen entfernen
    axis.text.y = element_text(face = "italic"), # labels nach Signifikanz färben: colour=s$col[order(s$mean)]
    legend.position = "none",
    axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0)), # Schriftgröße & Abstand Achsenlabels,
    axis.title.y = element_blank() # Schriftgröße & Abstand Achsenlabels,
  )+
  scale_colour_manual(values=c(ns="grey80",signif="black"))+
  geom_text(aes(x=signpos,label = signlvl), vjust=0.8,hjust=0)+ # vjust auf selbe Höhe wie Linie, hjust = linksbündig, 
  labs(x="Elevational shift [m]",y="Species")+
  scale_y_discrete(labels=s$taxon[order(s$mean)])+
  coord_cartesian(xlim = c(-250,248))+
  annotation_custom(
    grid::rasterGrob(
      image = readPNG("J:/Layout/Logo_hostplant.png"),
      x = .85, y =.035,
      width =.3))
```
...
library(patchwork)
((buttshifts + plot_spacer()) + inset_element(plantshifts, 0,0,1,1, align_to = "full")) + plot_layout(widths = c(1,1.5)) # plot_spacer() & inset_element, damit plant shifts über ganze Höhe gehen und nicht Legendenbereich oben frei lassen
```
...
hpl<-read.table("J:/Auswertung/Textfiles/hostplantlist.csv", sep=";", header=T)
...
plot(s+ss+sss~no.ind,data=a,ylim=c(min(s+ss+sss),max(no.species)), xaxt="n",yaxt="n",xlab=NA,ylab=NA,col="darkorange2", pch=19
     ); grid.raster(readPNG("J:/Layout/Logo_Ere_eur.png"), # Logo einfügen
            x = .08, y =.1,
            width =.1)
...
par(mfrow=c(2,1),mar=c(1,6,2,2)) # Breite Plotränder
...
plot(s+ss+sss~no.sb,data=m,ylim=c(min(s+ss+sss),max(m$no.species)),xlab = NA, xaxt="n", ylab = NA, yaxt = "n",col="darkorange2", pch=19
     ); grid.raster(readPNG("J:/Layout/Logo_hostplant.png"), # Logo einfügen
            x = .08, y =.08,
            width =.1)
...
# Temperaturunterschied pro 100 m
...
# Unterschied Tagesmittel 2009 - 2019 für Sommer / ganzes Jahr
...
ggplot(mt, aes(year, daymeanT))+
  ...
  theme(
    ...
    axis.title.x = element_text(size=12, margin = margin(t = 10, r = 0, b = 0, l = 0)),
    axis.title.y = element_text(size=12, margin = margin(t = 0, r = 10, b = 0, l = 0)),
    strip.background = element_blank(), # Hintergrund facet_labels
